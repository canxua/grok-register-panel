...
- **Secrets:** the ingest refuses to complete if anything shaped like a real key (GitHub token,
  ...
  prompts.
- **BM25 only in the hook.** It is the verified retrieval path for this project.
  Vector search is optional and currently disabled because QMD's packaged
  llama.cpp backend fails Metal shader compilation on this workstation. See
  `docs/TRELLIS.md` before re-enabling it.
