...
| Exact provider probe returns HTTP `200` but state becomes `verification_failed` | The probe allowed only 2 output tokens, so a valid bounded request could finish as `status=incomplete` and fail the strict validator | Use a small completion-sized budget of 16, keep rejecting generic `200` and `incomplete`, then replay the same credential | 2026-07-31 | `fa28bdf` |
| A Camoufox canary reaches the profile page but ends with `wait-cf:0` | The OVH-direct browser session did not clear the Turnstile round; this happens before SSO and does not test the auth bridge | Stop after one account, cool down, and retry only with a healthy sticky exit; do not batch-retry | 2026-07-31 | `CURRENT_STATE.md` |
